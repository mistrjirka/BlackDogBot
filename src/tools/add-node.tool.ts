...
async function probeUrlAsync(url: string, method: string = "HEAD"): Promise<{ reachable: boolean; error: string }> {
...
function hasTemplateVariables(url: string): boolean {
  return /\{\{.+?\}\}/.test(url);
}

async function validateFetcherConfigAsync(
  type: string,
