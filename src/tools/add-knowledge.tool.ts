...
export const addKnowledgeTool = tool(
  async ({ knowledge: knowledgeContent, collection, metadata }: { knowledge: string; collection: string; metadata: Record<string, unknown> }): Promise<{ id: string; success: boolean; error?: string }> => {
    try {
  ...
  },
  {
    ...
    schema: addKnowledgeToolInputSchema,
  },
);
