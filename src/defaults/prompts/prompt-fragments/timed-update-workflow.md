...
1. If the task ID is unknown, use `search_timed` to locate the relevant task.
2. Use `get_timed` to see the current task configuration and determine the schedule type (once or interval).
3. If the user asks to change instructions, logic, filtering rules, fetch mode, message format, or anything textual in the task, ALWAYS use `edit_instructions` with the COMPLETE new instructions text and `intention`
   - If the new instructions require different tools, include `tools` in the same `edit_instructions` call.
4. Use `edit_once` for non-instruction changes to one-time tasks (name, description, tools, runAt, notifyUser, enabled, messageDedupEnabled).
...
5. If the user reports missing periodic notifications (for example "the cron didn't send the morning summary"), dedup suppression is a likely cause:
...
6. Optionally call `get_timed` again to verify the update.
