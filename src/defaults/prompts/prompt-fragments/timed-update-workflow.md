...
3. Use `edit_once` for non-instruction changes to one-time tasks (name, description, tools, runAt, notifyUser, enabled).
   Use `edit_interval` for non-instruction changes to interval tasks (name, description, tools, every, offsetFromDayStart, timezone, notifyUser, enabled). When patching `every` or `offsetFromDayStart`, always provide both `hours` and `minutes`, including zero values (for example, use `{ hours: 2, minutes: 0 }`, not `{ hours: 2 }`).
