...
- **Scheduled time semantics:** For interval schedules, use human units: `every: { hours, minutes }` and optional `offsetFromDayStart: { hours, minutes }` with `timezone`. `offsetFromDayStart` is anchored to day start (00:00) in the schedule timezone, not task creation time. User-facing confirmations should show local human-readable time with timezone.
