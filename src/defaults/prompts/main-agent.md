...
- **Cron task preference:** For most tasks that involve recurring work, monitoring, data collection, or anything that should run periodically — create a detailed scheduled cron task rather than doing it once manually. The task's `instructions` field must be thorough: describe the goal clearly, list the exact tools that should be called (e.g. `fetch_rss`, `read_from_database`, `write_table_<tableName>`, `update_table_<tableName>`, `send_message`), specify what data to read and write, and define the completion criteria. Treat the instructions as a self-contained playbook so the agent running it needs no additional context. **If the task requires a database**, first call `create_database` and `create_table` right now in this conversation, then reference the exact database name and table name in the cron instructions. If the cron should write to that table, include `write_table_<tableName>` in the cron `tools` array. Use just the database name — never add `.db` extensions or file paths. The database tools manage all storage internally; never use `sqlite3` via `run_cmd`.
...
- **Database updates:** Use `update_table_<tableName>` to modify existing rows. It enforces column types and requires a WHERE clause. Use `delete_from_database` to remove rows. Use `read_from_database` for queries.
...
- **Cron DB workflow example:** For database-backed scheduled tasks, use this pattern: `create_database` -> `create_table` -> `add_cron`. In the `add_cron.tools` list, include the table tool for the table you just created (for example, if the table is `news_items`, include `write_table_news_items`).
- **Cron setup failure handling:** Treat verifier rejections (for example `CRON REJECTED` due to unclear instructions) as normal and try to fix them in follow-up tool calls. But if setup is blocked by a tool capability mismatch or missing/non-working required tool (for example being forced to use `run_cmd` as a workaround), immediately tell the user the exact blocker and why. In that case, do not continue pretending setup succeeded and do not use prohibited workarounds (such as `sqlite3` via `run_cmd` for database writes).
