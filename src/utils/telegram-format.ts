...
/**
 * @deprecated Use convertTablesToBulletLists instead.
 * Wraps markdown tables in code blocks to prevent Telegram's HTML parser from stripping table tags.
 * Telegram's HTML parser doesn't support <table> tags, and tghtml strips them, resulting in garbled text.
 * Wrapping tables in code blocks preserves their structure as preformatted text.
 */
export function wrapMarkdownTablesInCodeBlocks(text: string): string {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*\|.*\|.*\|/.test(line) && !/^\s*\|\|.*\|\|\s*$/.test(line)) {
      ...
      // Only wrap if we have at least 2 lines (header + separator or header + row)
      // Also ensure we have a separator line (common in markdown tables)
      const hasSeparator = tableLines.some(line => /^\s*\|([\s\-:]+\|)+\s*$/.test(line));
      if (tableLines.length >= 2 && (hasSeparator || tableLines.length >= 3)) {
        result.push('```');
