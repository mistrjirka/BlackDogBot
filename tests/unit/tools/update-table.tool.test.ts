...
describe("update_table tool", () => {
  ...
  it("converts 'now' to ISO timestamp for DATETIME columns with non-standard names", async () => {
    const tool = createUpdateTableTool("events", [
      { name: "id", type: "INTEGER", notNull: true, primaryKey: true, defaultValue: null },
      { name: "logged_when", type: "DATETIME", notNull: false, primaryKey: false, defaultValue: null },
      { name: "title", type: "TEXT", notNull: false, primaryKey: false, defaultValue: null },
    ]) as any;
    ...
    expect(result.success).toBe(true);
    const setArg = vi.mocked(litesql.updateTableAsync).mock.calls[0][2] as Record<string, unknown>;
    expect(typeof setArg.logged_when).toBe("string");
    expect((setArg.logged_when as string)).toContain("T");
    expect(setArg.logged_when).not.toBe("now");
  });
});
