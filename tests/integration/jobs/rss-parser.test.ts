...
<rss version="2.0">
  <channel>
    ...
    it("should parse description when content:encoded is not present", () => {
      const xml = `<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    ...
    <link>https://example.com</link>
    <item>
      <title>RT by @Archer83Able: Death toll from Israeli attack...</title>
      <dc:creator>@idreesali114</dc:creator>
      <description><![CDATA[<p>Death toll from Israeli attack at girls primary school in southern Iranian town of Minab rises to 40, state media says.</p>]]></description>
      <pubDate>Sat, 28 Feb 2026 12:40:27 GMT</pubDate>
      <guid isPermaLink="false">2027725583996616995</guid>
      <link>http://localhost:8080/idreesali114/status/2027725583996616995#m</link>
    </item>
  </channel>
</rss>`;
      const result = parseRssFeed(xml);
      ...
      expect(items[0].title).toContain("Death toll");
      expect(items[0].description).toContain("Death toll");
      expect(items[0]["dc:creator"]).toBe("@idreesali114");
      expect(items[0].guid).toBe("2027725583996616995");
    });

    it("should parse dc:creator as extra field", () => {
      const xml = `<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    <title>Test</title>
    ...
    <item>
      ...
      // The parser extracts ALL tags, so dc:creator should be present
      expect(items[0].title).toBe("Test Title");
      expect(items[0].description).toBe("Test description");
      expect(items[0]["dc:creator"]).toBe("@testuser");
    });
  });
});
